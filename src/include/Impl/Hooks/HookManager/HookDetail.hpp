...
struct HookResult
{
    ...
    inline void OrDie(const char* aMessage = nullptr) const
    {
        if (!success)
        {
            if (aMessage)
            {
                shared::raw::Assert::RaiseAssert(std::source_location::current(), aMessage);
            }
            else
            {
                if constexpr (sizeof(TOffset) == sizeof(uintptr_t))
                {
                    raw::Assert::RaiseAssert(std::source_location::current(), "Failed to hook at address {:#X}.", TOffset);
                }
                else
                {
                    raw::Assert::RaiseAssert(std::source_location::current(), "Failed to hook address id {}.", TOffset);
                }
            }
        }
    }
...
template<template<auto, typename> class TRaw, auto TOffset, typename TRawRet, typename... TRawArgs, typename TContext,
...
template<template<auto, typename> class TRaw, auto TOffset, typename TRet, typename... TArgs, typename TWrapper,
